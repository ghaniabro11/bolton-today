...
const AuthorTable: React.FC<AuthorTableProps> = ({
  authors,
  totalItems,
  currentPage,
  limit,
  ...
  const isAuthorUpdate = hasPermission(Permission.UPDATE_AUTHOR);
  const isAuthorDelete = hasPermission(Permission.DELETE_AUTHOR);
  ...
  const deleteAuthor = async (id: number) => {
    ...
    try {
      const res = await axiosInstance.delete(`/api/v1/authors/${id}`);
      if (res) {
        router.refresh();
              ...
              <div className="flex gap-2 justify-center">
                {isAuthorUpdate && (
                  <Button asChild variant="secondary" size="sm">
                    <Link href={`/admin/authors/update/${row?.id}`}>Edit</Link>
                  </Button>
                )}
                {isAuthorDelete && (
                  <Button
  ...
  return (
    <div className={className}>
      <div className="flex justify-between">
        ...
        <Button asChild>
          <Link href="/admin/authors/add">Add New</Link>
        </Button>
      </div>

      {/* Table */}
      <DataTable columns={columns} data={authors} loading={loading ?? false} />
      <div className="mt-3" />
      <Pagination
