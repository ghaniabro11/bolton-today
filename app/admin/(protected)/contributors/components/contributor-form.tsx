...
const ContributorForm: React.FC<CategoryProps> = ({
  categories,
  isUpdate = false,
    ...
    try {
      ...
      isUpdate
        ? (response = await axiosInstance.put(
            `/api/v1/contributors/${Number(id)}`,
            payload
          ))
        : (response = await axiosInstance.post("/api/v1/contributors", payload));
      if (response.data) {
        toast.success(response?.data?.message || "Operation successfull");
        router.push("/admin/contributors"); // Redirect to category list
      }
    } catch (error: any) {
      toast.error(
