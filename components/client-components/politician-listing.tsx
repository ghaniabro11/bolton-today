...
const PoliticianListingComponent = ({
  result,
}: {
  result: any;
}) => {
  const politicians = result?.data || [];
  ...
  return (
    <div className="py-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {politicians?.map((item: any) => (
          <Link
            key={item?.id}
            href={`/politician/${item?.slug}`}
...
export default PoliticianListingComponent;